...
static __global__ void _kernelDoubleMI(uint64_t numPairs,
		uint16_t numEntriesCases, uint16_t numEntriesCtrls, uint2 *devIds,
		GPUDoubleContTable *devDoubleTables, float *devMIValues, uint3 *devMiIds){
	...
	GPUDoubleContTable *table = &devDoubleTables[gid];
	uint32_t myId1 = devIds[gid].x;
	uint32_t myId2 = devIds[gid].y;

	uint16_t contCases[9];
	uint16_t contCtrls[9];
	...
	// Start calculating the values of the contingency table
	for(int i=0; i<numEntriesCases; i++){
		contCases[0] += _devpopcount(table->_cases00[i]);
		contCases[1] += _devpopcount(table->_cases01[i]);
		contCases[2] += _devpopcount(table->_cases02[i]);
		contCases[3] += _devpopcount(table->_cases10[i]);
		contCases[4] += _devpopcount(table->_cases11[i]);
		contCases[5] += _devpopcount(table->_cases12[i]);
		contCases[6] += _devpopcount(table->_cases20[i]);
		contCases[7] += _devpopcount(table->_cases21[i]);
		contCases[8] += _devpopcount(table->_cases22[i]);
	}
	for(int i=0; i<numEntriesCtrls; i++){
		contCtrls[0] += _devpopcount(table->_ctrls00[i]);
		contCtrls[1] += _devpopcount(table->_ctrls01[i]);
		contCtrls[2] += _devpopcount(table->_ctrls02[i]);
		contCtrls[3] += _devpopcount(table->_ctrls10[i]);
		contCtrls[4] += _devpopcount(table->_ctrls11[i]);
		contCtrls[5] += _devpopcount(table->_ctrls12[i]);
		contCtrls[6] += _devpopcount(table->_ctrls20[i]);
		contCtrls[7] += _devpopcount(table->_ctrls21[i]);
		contCtrls[8] += _devpopcount(table->_ctrls22[i]);
	}
	...
	devMIValues[gid] = entX+_entY-entAll;

	devMiIds[gid].x = myId1;
	devMiIds[gid].y = myId2;
	devMiIds[gid].z = 0;
}
...
static __global__ void _kernelTripleIG(uint64_t numPairs, uint32_t numSNPs,
	...
	for(; iterId3<numSNPs; iterId3+=blockDim.x){
		...
		float casex0 = 0.0;	float casex1 = 0.0;	float casex2 = 0.0;
		float casey0 = 0.0;	float casey1 = 0.0;	float casey2 = 0.0;
		float casez0 = 0.0;	float casez1 = 0.0;	float casez2 = 0.0;
		float casexy00 = 0.0;	float casexy01 = 0.0;	float casexy02 = 0.0;
		float casexy10 = 0.0;	float casexy11 = 0.0;	float casexy12 = 0.0;
		float casexy20 = 0.0;	float casexy21 = 0.0;	float casexy22 = 0.0;
		float casexz00 = 0.0;	float casexz01 = 0.0;	float casexz02 = 0.0;
		float casexz10 = 0.0;	float casexz11 = 0.0;	float casexz12 = 0.0;
		float casexz20 = 0.0;	float casexz21 = 0.0;	float casexz22 = 0.0;
		float caseyz00 = 0.0;	float caseyz01 = 0.0;	float caseyz02 = 0.0;
		float caseyz10 = 0.0;	float caseyz11 = 0.0;	float caseyz12 = 0.0;
		float caseyz20 = 0.0;	float caseyz21 = 0.0;	float caseyz22 = 0.0;
		float ctrlx0 = 0.0;	float ctrlx1 = 0.0;	float ctrlx2 = 0.0;
		float ctrly0 = 0.0;	float ctrly1 = 0.0;	float ctrly2 = 0.0;
		float ctrlz0 = 0.0;	float ctrlz1 = 0.0;	float ctrlz2 = 0.0;
		float ctrlxy00 = 0.0;	float ctrlxy01 = 0.0;	float ctrlxy02 = 0.0;
		float ctrlxy10 = 0.0;	float ctrlxy11 = 0.0;	float ctrlxy12 = 0.0;
		float ctrlxy20 = 0.0;	float ctrlxy21 = 0.0;	float ctrlxy22 = 0.0;
		float ctrlxz00 = 0.0;	float ctrlxz01 = 0.0;	float ctrlxz02 = 0.0;
		float ctrlxz10 = 0.0;	float ctrlxz11 = 0.0;	float ctrlxz12 = 0.0;
		float ctrlxz20 = 0.0;	float ctrlxz21 = 0.0;	float ctrlxz22 = 0.0;
		float ctrlyz00 = 0.0;	float ctrlyz01 = 0.0;	float ctrlyz02 = 0.0;
		float ctrlyz10 = 0.0;	float ctrlyz11 = 0.0;	float ctrlyz12 = 0.0;
		float ctrlyz20 = 0.0;	float ctrlyz21 = 0.0;	float ctrlyz22 = 0.0;
		...
		casex0 += pCase;	casey0 += pCase;	casez0 += pCase;
		casexy00 += pCase;	casexz00 += pCase;	caseyz00 += pCase;

		pCtrl = tripleCtrls[0]*_invInds;
		if(pCtrl != 0.0){
			entAll -= pCtrl*log2(pCtrl);
		}
		ctrlx0 += pCtrl;	ctrly0 += pCtrl;	ctrlz0 += pCtrl;
		ctrlxy00 += pCtrl;	ctrlxz00 += pCtrl;	ctrlyz00 += pCtrl;
		...
		casex0 += pCase;	casey0 += pCase;	casez1 += pCase;
		casexy00 += pCase;	casexz01 += pCase;	caseyz01 += pCase;

		pCtrl = tripleCtrls[1]*_invInds;
		if(pCtrl != 0.0){
			entAll -= pCtrl*log2(pCtrl);
		}
		ctrlx0 += pCtrl;	ctrly0 += pCtrl;	ctrlz1 += pCtrl;
		ctrlxy00 += pCtrl;	ctrlxz01 += pCtrl;	ctrlyz01 += pCtrl;
		...
		casex0 += pCase;	casey0 += pCase;	casez2 += pCase;
		casexy00 += pCase;	casexz02 += pCase;	caseyz02 += pCase;

		pCtrl = tripleCtrls[2]*_invInds;
		if(pCtrl != 0.0){
			entAll -= pCtrl*log2(pCtrl);
		}
		ctrlx0 += pCtrl;	ctrly0 += pCtrl;	ctrlz2 += pCtrl;
		ctrlxy00 += pCtrl;	ctrlxz02 += pCtrl;	ctrlyz02 += pCtrl;
		...
		casex0 += pCase;	casey1 += pCase;	casez0 += pCase;
		casexy01 += pCase;	casexz00 += pCase;	caseyz10 += pCase;

		pCtrl = tripleCtrls[3]*_invInds;
		if(pCtrl != 0.0){
			entAll -= pCtrl*log2(pCtrl);
		}
		ctrlx0 += pCtrl;	ctrly1 += pCtrl;	ctrlz0 += pCtrl;
		ctrlxy01 += pCtrl;	ctrlxz00 += pCtrl;	ctrlyz10 += pCtrl;
		...
		casex0 += pCase;	casey1 += pCase;	casez1 += pCase;
		casexy01 += pCase;	casexz01 += pCase;	caseyz11 += pCase;

		pCtrl = tripleCtrls[4]*_invInds;
		if(pCtrl != 0.0){
			entAll -= pCtrl*log2(pCtrl);
		}
		ctrlx0 += pCtrl;	ctrly1 += pCtrl;	ctrlz1 += pCtrl;
		ctrlxy01 += pCtrl;	ctrlxz01 += pCtrl;	ctrlyz11 += pCtrl;
		...
		casex0 += pCase;	casey1 += pCase;	casez2 += pCase;
		casexy01 += pCase;	casexz02 += pCase;	caseyz12 += pCase;

		pCtrl = tripleCtrls[5]*_invInds;
		if(pCtrl != 0.0){
			entAll -= pCtrl*log2(pCtrl);
		}
		ctrlx0 += pCtrl;	ctrly1 += pCtrl;	ctrlz2 += pCtrl;
		ctrlxy01 += pCtrl;	ctrlxz02 += pCtrl;	ctrlyz12 += pCtrl;
		...
		casex0 += pCase;	casey2 += pCase;	casez0 += pCase;
		casexy02 += pCase;	casexz00 += pCase;	caseyz20 += pCase;

		pCtrl = tripleCtrls[6]*_invInds;
		if(pCtrl != 0.0){
			entAll -= pCtrl*log2(pCtrl);
		}
		ctrlx0 += pCtrl;	ctrly2 += pCtrl;	ctrlz0 += pCtrl;
		ctrlxy02 += pCtrl;	ctrlxz00 += pCtrl;	ctrlyz20 += pCtrl;
		...
		casex0 += pCase;	casey2 += pCase;	casez1 += pCase;
		casexy02 += pCase;	casexz01 += pCase;	caseyz21 += pCase;

		pCtrl = tripleCtrls[7]*_invInds;
		if(pCtrl != 0.0){
			entAll -= pCtrl*log2(pCtrl);
		}
		ctrlx0 += pCtrl;	ctrly2 += pCtrl;	ctrlz1 += pCtrl;
		ctrlxy02 += pCtrl;	ctrlxz01 += pCtrl;	ctrlyz21 += pCtrl;
		...
		casex0 += pCase;	casey2 += pCase;	casez2 += pCase;
		casexy02 += pCase;	casexz02 += pCase;	caseyz22 += pCase;

		pCtrl = tripleCtrls[8]*_invInds;
		if(pCtrl != 0.0){
			entAll -= pCtrl*log2(pCtrl);
		}
		ctrlx0 += pCtrl;	ctrly2 += pCtrl;	ctrlz2 += pCtrl;
		ctrlxy02 += pCtrl;	ctrlxz02 += pCtrl;	ctrlyz22 += pCtrl;
		...
		casex1 += pCase;	casey0 += pCase;	casez0 += pCase;
		casexy10 += pCase;	casexz10 += pCase;	caseyz00 += pCase;

		pCtrl = tripleCtrls[9]*_invInds;
		if(pCtrl != 0.0){
			entAll -= pCtrl*log2(pCtrl);
		}
		ctrlx1 += pCtrl;	ctrly0 += pCtrl;	ctrlz0 += pCtrl;
		ctrlxy10 += pCtrl;	ctrlxz10 += pCtrl;	ctrlyz00 += pCtrl;
		...
		casex1 += pCase;	casey0 += pCase;	casez1 += pCase;
		casexy10 += pCase;	casexz11 += pCase;	caseyz01 += pCase;

		pCtrl = tripleCtrls[10]*_invInds;
		if(pCtrl != 0.0){
			entAll -= pCtrl*log2(pCtrl);
		}
		ctrlx1 += pCtrl;	ctrly0 += pCtrl;	ctrlz1 += pCtrl;
		ctrlxy10 += pCtrl;	ctrlxz11 += pCtrl;	ctrlyz01 += pCtrl;
		...
		casex1 += pCase;	casey0 += pCase;	casez2 += pCase;
		casexy10 += pCase;	casexz12 += pCase;	caseyz02 += pCase;

		pCtrl = tripleCtrls[11]*_invInds;
		if(pCtrl != 0.0){
			entAll -= pCtrl*log2(pCtrl);
		}
		ctrlx1 += pCtrl;	ctrly0 += pCtrl;	ctrlz2 += pCtrl;
		ctrlxy10 += pCtrl;	ctrlxz12 += pCtrl;	ctrlyz02 += pCtrl;
		...
		casex1 += pCase;	casey1 += pCase;	casez0 += pCase;
		casexy11 += pCase;	casexz10 += pCase;	caseyz10 += pCase;

		pCtrl = tripleCtrls[12]*_invInds;
		if(pCtrl != 0.0){
			entAll -= pCtrl*log2(pCtrl);
		}
		ctrlx1 += pCtrl;	ctrly1 += pCtrl;	ctrlz0 += pCtrl;
		ctrlxy11 += pCtrl;	ctrlxz10 += pCtrl;	ctrlyz10 += pCtrl;
		...
		casex1 += pCase;	casey1 += pCase;	casez1 += pCase;
		casexy11 += pCase;	casexz11 += pCase;	caseyz11 += pCase;

		pCtrl = tripleCtrls[13]*_invInds;
		if(pCtrl != 0.0){
			entAll -= pCtrl*log2(pCtrl);
		}
		ctrlx1 += pCtrl;	ctrly1 += pCtrl;	ctrlz1 += pCtrl;
		ctrlxy11 += pCtrl;	ctrlxz11 += pCtrl;	ctrlyz11 += pCtrl;
		...
		casex1 += pCase;	casey1 += pCase;	casez2 += pCase;
		casexy11 += pCase;	casexz12 += pCase;	caseyz12 += pCase;

		pCtrl = tripleCtrls[14]*_invInds;
		if(pCtrl != 0.0){
			entAll -= pCtrl*log2(pCtrl);
		}
		ctrlx1 += pCtrl;	ctrly1 += pCtrl;	ctrlz2 += pCtrl;
		ctrlxy11 += pCtrl;	ctrlxz12 += pCtrl;	ctrlyz12 += pCtrl;
		...
		casex1 += pCase;	casey2 += pCase;	casez0 += pCase;
		casexy12 += pCase;	casexz10 += pCase;	caseyz20 += pCase;

		pCtrl = tripleCtrls[15]*_invInds;
		if(pCtrl != 0.0){
			entAll -= pCtrl*log2(pCtrl);
		}
		ctrlx1 += pCtrl;	ctrly2 += pCtrl;	ctrlz0 += pCtrl;
		ctrlxy12 += pCtrl;	ctrlxz10 += pCtrl;	ctrlyz20 += pCtrl;
		...
		casex1 += pCase;	casey2 += pCase;	casez1 += pCase;
		casexy12 += pCase;	casexz11 += pCase;	caseyz21 += pCase;

		pCtrl = tripleCtrls[16]*_invInds;
		if(pCtrl != 0.0){
			entAll -= pCtrl*log2(pCtrl);
		}
		ctrlx1 += pCtrl;	ctrly2 += pCtrl;	ctrlz1 += pCtrl;
		ctrlxy12 += pCtrl;	ctrlxz11 += pCtrl;	ctrlyz21 += pCtrl;
		...
		casex1 += pCase;	casey2 += pCase;	casez2 += pCase;
		casexy12 += pCase;	casexz12 += pCase;	caseyz22 += pCase;

		pCtrl = tripleCtrls[17]*_invInds;
		if(pCtrl != 0.0){
			entAll -= pCtrl*log2(pCtrl);
		}
		ctrlx1 += pCtrl;	ctrly2 += pCtrl;	ctrlz2 += pCtrl;
		ctrlxy12 += pCtrl;	ctrlxz12 += pCtrl;	ctrlyz22 += pCtrl;
		...
		casex2 += pCase;	casey0 += pCase;	casez0 += pCase;
		casexy20 += pCase;	casexz20 += pCase;	caseyz00 += pCase;

		pCtrl = tripleCtrls[18]*_invInds;
		if(pCtrl != 0.0){
			entAll -= pCtrl*log2(pCtrl);
		}
		ctrlx2 += pCtrl;	ctrly0 += pCtrl;	ctrlz0 += pCtrl;
		ctrlxy20 += pCtrl;	ctrlxz20 += pCtrl;	ctrlyz00 += pCtrl;
		...
		casex2 += pCase;	casey0 += pCase;	casez1 += pCase;
		casexy20 += pCase;	casexz21 += pCase;	caseyz01 += pCase;

		pCtrl = tripleCtrls[19]*_invInds;
		if(pCtrl != 0.0){
			entAll -= pCtrl*log2(pCtrl);
		}
		ctrlx2 += pCtrl;	ctrly0 += pCtrl;	ctrlz1 += pCtrl;
		ctrlxy20 += pCtrl;	ctrlxz21 += pCtrl;	ctrlyz01 += pCtrl;
		...
		casex2 += pCase;	casey0 += pCase;	casez2 += pCase;
		casexy20 += pCase;	casexz22 += pCase;	caseyz02 += pCase;

		pCtrl = tripleCtrls[20]*_invInds;
		if(pCtrl != 0.0){
			entAll -= pCtrl*log2(pCtrl);
		}
		ctrlx2 += pCtrl;	ctrly0 += pCtrl;	ctrlz2 += pCtrl;
		ctrlxy20 += pCtrl;	ctrlxz22 += pCtrl;	ctrlyz02 += pCtrl;
		...
		casex2 += pCase;	casey1 += pCase;	casez0 += pCase;
		casexy21 += pCase;	casexz20 += pCase;	caseyz10 += pCase;

		pCtrl = tripleCtrls[21]*_invInds;
		if(pCtrl != 0.0){
			entAll -= pCtrl*log2(pCtrl);
		}
		ctrlx2 += pCtrl;	ctrly1 += pCtrl;	ctrlz0 += pCtrl;
		ctrlxy21 += pCtrl;	ctrlxz20 += pCtrl;	ctrlyz10 += pCtrl;
		...
		casex2 += pCase;	casey1 += pCase;	casez1 += pCase;
		casexy21 += pCase;	casexz21 += pCase;	caseyz11 += pCase;

		pCtrl = tripleCtrls[22]*_invInds;
		if(pCtrl != 0.0){
			entAll -= pCtrl*log2(pCtrl);
		}
		ctrlx2 += pCtrl;	ctrly1 += pCtrl;	ctrlz1 += pCtrl;
		ctrlxy21 += pCtrl;	ctrlxz21 += pCtrl;	ctrlyz11 += pCtrl;
		...
		casex2 += pCase;	casey1 += pCase;	casez2 += pCase;
		casexy21 += pCase;	casexz22 += pCase;	caseyz12 += pCase;

		pCtrl = tripleCtrls[23]*_invInds;
		if(pCtrl != 0.0){
			entAll -= pCtrl*log2(pCtrl);
		}
		ctrlx2 += pCtrl;	ctrly1 += pCtrl;	ctrlz2 += pCtrl;
		ctrlxy21 += pCtrl;	ctrlxz22 += pCtrl;	ctrlyz12 += pCtrl;
		...
		casex2 += pCase;	casey2 += pCase;	casez0 += pCase;
		casexy22 += pCase;	casexz20 += pCase;	caseyz20 += pCase;

		pCtrl = tripleCtrls[24]*_invInds;
		if(pCtrl != 0.0){
			entAll -= pCtrl*log2(pCtrl);
		}
		ctrlx2 += pCtrl;	ctrly2 += pCtrl;	ctrlz0 += pCtrl;
		ctrlxy22 += pCtrl;	ctrlxz20 += pCtrl;	ctrlyz20 += pCtrl;
		...
		casex2 += pCase;	casey2 += pCase;	casez1 += pCase;
		casexy22 += pCase;	casexz21 += pCase;	caseyz21 += pCase;

		pCtrl = tripleCtrls[25]*_invInds;
		if(pCtrl != 0.0){
			entAll -= pCtrl*log2(pCtrl);
		}
		ctrlx2 += pCtrl;	ctrly2 += pCtrl;	ctrlz1 += pCtrl;
		ctrlxy22 += pCtrl;	ctrlxz21 += pCtrl;	ctrlyz21 += pCtrl;
		...
		casex2 += pCase;	casey2 += pCase;	casez2 += pCase;
		casexy22 += pCase;	casexz22 += pCase;	caseyz22 += pCase;

		pCtrl = tripleCtrls[26]*_invInds;
		if(pCtrl != 0.0){
			entAll -= pCtrl*log2(pCtrl);
		}
		ctrlx2 += pCtrl;	ctrly2 += pCtrl;	ctrlz2 += pCtrl;
		ctrlxy22 += pCtrl;	ctrlxz22 += pCtrl;	ctrlyz22 += pCtrl;
...
GPUEngine::GPUEngine(Options *options) {
	...
	cudaMallocHost(&_hostMIValues, NUM_PAIRS_BLOCK*options->getNumOutputs()*sizeof(float));
	myCheckCudaError;

	cudaMallocHost(&_hostMiIds, NUM_PAIRS_BLOCK*options->getNumOutputs()*sizeof(uint3));
	myCheckCudaError;
}
...
void GPUEngine::setNums(uint32_t numSNPs, uint16_t numCases, uint16_t numCtrls){
...
void GPUEngine::initialize(int tid){

	_gpuId = _options->getGPUId(tid);
	...
	// Increase the size of the L1 compared to shared memory
	cudaFuncSetCacheConfig(_kernelDoubleMI, cudaFuncCachePreferL1);
	myCheckCudaError;
...
void GPUEngine::loadSNPs(uint32_t* host0Cases, uint32_t* host1Cases, uint32_t* host2Cases,
...
void GPUEngine::mutualInfo(uint64_t numPairs, uint2* ids, MutualInfo* mutualInfo,
			float& minMI, uint16_t& minMIPos, uint16_t& numEntriesWithMI){

	uint16_t numOutputs = _options->getNumOutputs();
	...
	sharedSize += numOutputs*NUM_TH_PER_BLOCK*(sizeof(float)+sizeof(uint32_t));
	...
	if(_options->isMI()){
		_kernelTripleMI<<<gridMI, blockMI, sharedSize>>>(numPairs, _numSNPs, _numEntriesCase,_numEntriesCtrl,
			...
			_devDoubleTables, numOutputs, _devMIValues, _devMiIds);
	} else{
	...
	cudaMemcpy(_hostMIValues, _devMIValues, numPairs*numOutputs*sizeof(float),  cudaMemcpyDeviceToHost);
	myCheckCudaError;
	cudaMemcpy(_hostMiIds, _devMiIds, numPairs*numOutputs*sizeof(uint3),  cudaMemcpyDeviceToHost);
	myCheckCudaError;

	_findNHighestMI(mutualInfo, numPairs*numOutputs, minMI, minMIPos, numEntriesWithMI);
}


void GPUEngine::_findNHighestMI(MutualInfo* mutualInfo, uint64_t totalValues,
	float& minMI, uint16_t& minMIPos,uint16_t& numEntriesWithMI){
	...
	if(numEntriesWithMI == 0){ // The first values are directly stored
		for(iter=0; iter<numOutputs; iter++){
