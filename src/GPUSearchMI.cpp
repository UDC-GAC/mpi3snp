...
void GPUSearchMI::execute() {
    ...
    for (int tid = 0; tid < _options->getNumGPUs(); tid++) {
        _engines[tid] = new GPUEngine(_options, tid, distributor->getNumSnp(), distributor->getNumCases(),
                                      distributor->getNumCtrls(), distributor->getHost0Cases(),
                                      distributor->getHost1Cases(), distributor->getHost2Cases(),
                                      distributor->getHost0Ctrls(), distributor->getHost1Ctrls(),
                                      distributor->getHost2Ctrls());
...
void *GPUSearchMI::_GPUMI(void *arg) {
    ...
    MutualInfo *mutualInfo = new MutualInfo[params->_numOutputs];
    ...
    memcpy(params->_mutualInfo, mutualInfo, params->_numOutputs * sizeof(MutualInfo));
